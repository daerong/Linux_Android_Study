...
//static ssize_t iom_text_lcd_read(struct file *file, char *buf, size_t count, loff_t *f_pos);
...
struct file_operations iom_text_lcd_fops = {
	...
	//.read = iom_text_lcd_read,
	.release = iom_text_lcd_release
};

static int iom_text_lcd_open(struct inode *inode, struct file *file) {
	if (text_lcd_port_usage) {				// 정상 종료 시 0을 반환
		return -EBUSY;
	}
...
MODULE_DESCRIPTION("FPGA TEXT LCD driver");